ALTER TABLE public.bowler_stats_cache
  ADD COLUMN IF NOT EXISTS pins_lost integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS pins_lost_per_game numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS complete_games integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS score_stddev numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS first5_avg numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS last5_avg numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS big_opening_avg numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS big_finish_avg numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS clutch_marks integer NOT NULL DEFAULT 0;

ALTER TABLE public.team_stats_cache
  ADD COLUMN IF NOT EXISTS pins_lost integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS pins_lost_per_game numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS team_games integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS score_stddev numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS first5_avg numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS last5_avg numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS big_opening_avg numeric NOT NULL DEFAULT 0,
...
BEGIN
  ...
  FOREACH s IN ARRAY scopes LOOP
    ...
    WITH tm AS (
      ...
      SELECT gc.team_id, gc.match_id, gc.game_number, gc.game_id, gc.scratch_score,
      ...
      FROM public.v_game_context gc
      JOIN public.bowler_games bg ON bg.id = gc.game_id AND bg.is_complete
      JOIN public.frames f ON f.game_id = gc.game_id
      WHERE gc.season_id = p_season_id AND gc.match_status = 'final' AND gc.is_blind = false
        AND (s = 'full' OR gc.third = substring(s from 7)::int)
      GROUP BY gc.team_id, gc.match_id, gc.game_number, gc.game_id, gc.scratch_score
    ), tgseg AS (
      SELECT team_id, match_id, game_number,
             SUM(COALESCE(c5,0)) AS f5,
             SUM(scratch_score - COALESCE(c5,0)) AS l5,
             SUM(COALESCE(c3,0)) AS bo,
             SUM(scratch_score - COALESCE(c7,0)) AS bf
      FROM tgs GROUP BY team_id, match_id, game_number
    ), tsegagg AS (
      SELECT team_id,
    ...
    INSERT INTO public.team_stats_cache
      (season_id, team_id, scope, matches, points, points_possible, scratch_pinfall, hdcp_pinfall,
       ...
       first5_avg, last5_avg, big_opening_avg, big_finish_avg, clutch_marks)
    ...
    WITH g AS (
      ...
      SELECT g.bowler_id, g.game_id, g.scratch_score,
             ...
             MAX(f.cumulative_score) FILTER (WHERE f.frame_number = 7) AS c7
      FROM g
      JOIN public.bowler_games bg ON bg.id = g.game_id AND bg.is_complete
      ...
      SELECT bowler_id, COUNT(*) AS complete_games,
             ROUND(AVG(COALESCE(c5,0))::numeric, 2) AS first5_avg,
             ROUND(AVG(scratch_score - COALESCE(c5,0))::numeric, 2) AS last5_avg,
             ROUND(AVG(COALESCE(c3,0))::numeric, 2) AS big_opening_avg,
             ROUND(AVG(scratch_score - COALESCE(c7,0))::numeric, 2) AS big_finish_avg,
